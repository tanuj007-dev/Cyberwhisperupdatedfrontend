...
import { Loader2, Users, Mail, Phone, Calendar, BookOpen } from 'lucide-react';

export default function UpcomingEnrollmentsPage() {
    ...
    return (
        <div className="p-8">
            <div className="mb-8">
                <h1 className="text-3xl sm:text-4xl md:text-[50px] font-semibold tracking-tight leading-tight text-black">Batch Enrollments</h1>
                <p className="text-gray-600 dark:text-gray-400 mt-2">Enrollments from the Enroll Now button on batches</p>
            </div>
            ...
            {enrollments.length === 0 && !loading ? (
                <div className="text-center py-16 bg-gray-50 dark:bg-gray-900 rounded-2xl">
                    ...
                    <p className="text-gray-600 dark:text-gray-400">Batch enrollments will appear here when users click Enroll Now.</p>
                </div>
            ) : (
                <div className="space-y-6">
                    {enrollments.map((row) => (
                        <div
                            key={row.id ?? `${row.email}-${row.batch_name || row.program_name}`}
                                ...
                                <div className="flex flex-wrap items-start gap-4">
                                    <div className="flex-1 min-w-0 space-y-3">
                                        <div className="flex flex-wrap items-center gap-2">
                                            ...
                                            {row.start_date && (
                                                <span className="inline-flex items-center gap-1 px-2 py-1 bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-200 rounded-lg text-sm font-medium">
                                                    <Calendar size={14} />
                                                    Starts: {new Date(row.start_date).toLocaleDateString()}
                                                </span>
                                            )}
                                        </div>
                                        <div className="flex flex-wrap items-center gap-2">
