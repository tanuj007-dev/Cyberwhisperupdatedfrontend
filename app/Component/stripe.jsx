...
export default function Stripe() {
    ...
    return (
        <nav className="w-full bg-white border-y border-gray-100 py-2 font-sans">
            <div className="container mx-auto px-6 max-w-7xl">
                <div className="flex flex-col sm:flex-row items-center justify-between gap-6 sm:gap-4 text-[16px]">

                    {/* Label Section */}
                    <div className="flex items-center gap-2">
                        <span className="font-bold text-[#1a1a2e] whitespace-nowrap">Categories:</span>
                    </div>
                        ...
                        {categories.map((cat) => (
                            <button
                                ...
                                className={`relative whitespace-nowrap font-bold transition-all duration-300 py-1 ${activeTab === cat.id
                                    ? 'text-[#6B46E5]'
                                    : 'text-slate-900/60 hover:text-[#6B46E5]/80'
                                    }`}
                            >
                                {cat.label}
